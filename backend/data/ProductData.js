// product.js:

function getRandom(arr) {
  if (!Array.isArray(arr) || arr.length === 0) return null;
  return arr[Math.floor(Math.random() * arr.length)];
}

const ALLOWED_BRANDS = [
  "Urban Threads", "Modern Fit", "Street Style", "Beach Breeze", "Fashionista", "ChicStyle"
];
const ALLOWED_MATERIALS = [
  "Cotton", "Wool", "Denim", "Polyster", "Silk", "Linen", "Viscose", "Fleece"
];
function getRandomAllowed(arr) {
  return arr[Math.floor(Math.random() * arr.length)];
}
const products = [
  {
    "name": "Classic Oxford Button-Down Shirt",
    "description": "This classic Oxford shirt is tailored for a polished yet casual look. Crafted from high-quality cotton, it features a button-down collar and a comfortable, slightly relaxed fit. Perfect for both formal and casual occasions, it comes with long sleeves, a button placket, and a yoke at the back. The shirt is finished with a gently rounded hem and adjustable button cuffs.",
    "price": 39.99,
    "discountPrice": 34.99,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Urban Threads",
    "sizes": [
      "S",
      "M",
      "L",
      "XL",
      "XXL"
    ],
    "material": "Cotton",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/769749/pexels-photo-769749.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Classic Oxford Button-Down Shirt Front View",
        "imageId": ""
      }
    ],
    "rating": 4.5,
    "numReviews": 12
  },
  {
    "name": "Slim-Fit Stretch Shirt",
    "description": "A versatile slim-fit shirt perfect for business or evening events. Designed with a fitted silhouette, the added stretch provides maximum comfort throughout the day. Features a crisp turn-down collar, button placket, and adjustable cuffs.",
    "price": 29.99,
    "discountPrice": 24.99,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Modern Fit",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton Blend",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28915333/pexels-photo-28915333/free-photo-of-stylish-man-in-hat-at-sanssouci-palace.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Slim-Fit Stretch Shirt Front View",
        "imageId": ""
      }
    ],
    "rating": 4.8,
    "numReviews": 15
  },
  {
    "name": "Casual Denim Shirt",
    "description": "This casual denim shirt is made from lightweight cotton denim. It features a regular fit, snap buttons, and a straight hem. With Western-inspired details, this shirt is perfect for layering or wearing solo.",
    "price": 49.99,
    "discountPrice": 44.99,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Street Style",
    "sizes": [
      "S",
      "M",
      "L",
      "XL",
      "XXL"
    ],
    "material": "Denim",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/20715516/pexels-photo-20715516/free-photo-of-african-man-posing-in-denim-outfit.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Casual Denim Shirt Front View",
        "imageId": ""
      }
    ],
    "rating": 4.6,
    "numReviews": 8
  },
  {
    "name": "Printed Resort Shirt",
    "description": "Designed for summer, this printed resort shirt is perfect for vacation or weekend getaways. It features a relaxed fit, short sleeves, and a camp collar. The all-over tropical print adds a playful vibe.",
    "price": 29.99,
    "discountPrice": 22.99,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Beach Breeze",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Viscose",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/2403586/pexels-photo-2403586.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2",
        "altText": "Printed Resort Shirt Front View",
        "imageId": ""
      }
    ],
    "rating": 4.4,
    "numReviews": 10
  },
  {
    "name": "Slim-Fit Easy-Iron Shirt",
    "description": "A slim-fit, easy-iron shirt in woven cotton fabric with a fitted silhouette. Features a turn-down collar, classic button placket, and a yoke at the back. Long sleeves and adjustable button cuffs with a rounded hem.",
    "price": 34.99,
    "discountPrice": 29.99,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Urban Chic",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28713553/pexels-photo-28713553/free-photo-of-confident-young-man-outdoors-at-dusk.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Slim-Fit Easy-Iron Shirt Front View",
        "imageId": ""
      }
    ],
    "rating": 5,
    "numReviews": 14
  },
  {
    "name": "Polo T-Shirt with Ribbed Collar",
    "description": "A wardrobe classic, this polo t-shirt features a ribbed collar and cuffs. Made from 100% cotton, it offers breathability and comfort throughout the day. Tailored in a slim fit with a button placket at the neckline.",
    "price": 24.99,
    "discountPrice": 19.99,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Polo Classics",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/3228934/pexels-photo-3228934.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Polo T-Shirt Front View",
        "imageId": ""
      }
    ],
    "rating": 4.3,
    "numReviews": 22
  },
  {
    "name": "Oversized Graphic T-Shirt",
    "description": "An oversized graphic t-shirt that combines comfort with street style. Featuring bold prints across the chest, this relaxed fit tee offers a modern vibe, perfect for pairing with jeans or joggers.",
    "price": 19.99,
    "discountPrice": 15.99,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Street Vibes",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/7541670/pexels-photo-7541670.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Oversized Graphic T-Shirt Front View",
        "imageId": ""
      }
    ],
    "rating": 4.6,
    "numReviews": 30
  },
  {
    "name": "Regular-Fit Henley Shirt",
    "description": "A modern take on the classic Henley shirt, this regular-fit style features a buttoned placket and ribbed cuffs. Made from a soft cotton blend with a touch of elastane for stretch.",
    "price": 22.99,
    "discountPrice": 18.99,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Heritage Wear",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton Blend",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/14254077/pexels-photo-14254077.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Regular-Fit Henley Shirt Front View",
        "imageId": ""
      }
    ],
    "rating": 4.5,
    "numReviews": 25
  },
  {
    "name": "Long-Sleeve Thermal Tee",
    "description": "Stay warm with this long-sleeve thermal tee, made from soft cotton with a waffle-knit texture. Ideal for layering in cooler months, the slim-fit design ensures a snug yet comfortable fit.",
    "price": 27.99,
    "discountPrice": 22.99,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Winter Basics",
    "sizes": [
      "S",
      "M",
      "L",
      "XL",
      "XXL"
    ],
    "material": "Cotton",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/5037615/pexels-photo-5037615.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Long-Sleeve Thermal Tee Front View",
        "imageId": ""
      }
    ],
    "rating": 4.4,
    "numReviews": 18
  },
  {
    "name": "V-Neck Classic T-Shirt",
    "description": "A classic V-neck t-shirt for everyday wear. This regular-fit tee is made from breathable cotton and features a clean, simple design with a flattering V-neckline. Lightweight fabric and soft texture make it perfect for casual looks.",
    "price": 14.99,
    "discountPrice": 11.99,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Everyday Comfort",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/18899631/pexels-photo-18899631/free-photo-of-man-posing-in-white-v-neck-knitted-vest.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "V-Neck Classic T-Shirt Front View",
        "imageId": ""
      }
    ],
    "rating": 4.7,
    "numReviews": 28
  },
  {
    "name": "Slim Fit Joggers",
    "description": "Slim-fit joggers with an elasticated drawstring waist. Features ribbed hems and side pockets. Ideal for casual outings or workouts.",
    "price": 40,
    "discountPrice": 35,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "ActiveWear",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton Blend",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/12044905/pexels-photo-12044905.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Slim Fit Joggers Front View",
        "imageId": ""
      }
    ],
    "rating": 4.5,
    "numReviews": 12
  },
  {
    "name": "Cargo Joggers",
    "description": "Relaxed-fit cargo joggers featuring multiple pockets for functionality. Drawstring waist and cuffed hems for a modern look.",
    "price": 45,
    "discountPrice": 40,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "UrbanStyle",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/27988881/pexels-photo-27988881/free-photo-of-a-man-in-a-white-shirt-and-grey-pants-leaning-against-a-wall.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Cargo Joggers Front View",
        "imageId": ""
      }
    ],
    "rating": 4.7,
    "numReviews": 20
  },
  {
    "name": "Tapered Sweatpants",
    "description": "Tapered sweatpants designed for comfort. Elastic waistband with adjustable drawstring, perfect for lounging or athletic activities.",
    "price": 35,
    "discountPrice": 30,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "ChillZone",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Fleece",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28960365/pexels-photo-28960365/free-photo-of-man-standing-on-scenic-road-in-autumn-setting.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Tapered Sweatpants Front View",
        "imageId": ""
      }
    ],
    "rating": 4.3,
    "numReviews": 18
  },
  {
    "name": "Denim Jeans",
    "description": "Classic slim-fit denim jeans with a slight stretch for comfort. Features a zip fly and five-pocket styling for a timeless look.",
    "price": 60,
    "discountPrice": 50,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "DenimCo",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Denim",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/27045991/pexels-photo-27045991/free-photo-of-young-man-wearing-white-polo-shirt-and-gray-suit-trousers-leaning-against-concrete-column.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Denim Jeans Front View",
        "imageId": ""
      }
    ],
    "rating": 4.6,
    "numReviews": 22
  },
  {
    "name": "Chino Pants",
    "description": "Slim-fit chino pants made from stretch cotton twill. Features a button closure and front and back pockets. Ideal for both casual and semi-formal wear.",
    "price": 55,
    "discountPrice": 48,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "CasualLook",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28086474/pexels-photo-28086474/free-photo-of-man-leaning-on-vintage-car-in-urban-setting.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Chino Pants Front View",
        "imageId": ""
      }
    ],
    "rating": 4.8,
    "numReviews": 15
  },
  {
    "name": "Track Pants",
    "description": "Comfortable track pants with an elasticated waistband and tapered leg. Features side stripes for a sporty look. Ideal for athletic and casual wear.",
    "price": 40,
    "discountPrice": 35,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "SportX",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Polyester",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28945175/pexels-photo-28945175/free-photo-of-young-man-in-stylish-casual-outfit-outdoors.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Track Pants Front View",
        "imageId": ""
      }
    ],
    "rating": 4.2,
    "numReviews": 17
  },
  {
    "name": "Slim Fit Trousers",
    "description": "Tailored slim-fit trousers with belt loops and a hook-and-eye closure. Suitable for formal occasions or smart-casual wear.",
    "price": 65,
    "discountPrice": 55,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "ExecutiveStyle",
    "sizes": [
      "M",
      "L",
      "XL"
    ],
    "material": "Polyester",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28912858/pexels-photo-28912858/free-photo-of-urban-male-street-portrait-in-casablanca.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Slim Fit Trousers Front View",
        "imageId": ""
      }
    ],
    "rating": 4.7,
    "numReviews": 10
  },
  {
    "name": "Cargo Pants",
    "description": "Loose-fit cargo pants with multiple utility pockets. Features adjustable ankle cuffs and a drawstring waist for versatility and comfort.",
    "price": 50,
    "discountPrice": 45,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "StreetWear",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/27364090/pexels-photo-27364090/free-photo-of-portrait-of-man-in-sunglasses-and-a-black-shirt-against-the-blue-sky.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Cargo Pants Front View",
        "imageId": ""
      }
    ],
    "rating": 4.5,
    "numReviews": 13
  },
  {
    "name": "Relaxed Fit Sweatpants",
    "description": "Relaxed-fit sweatpants made from soft fleece fabric. Features an elastic waist and adjustable drawstring for a custom fit.",
    "price": 35,
    "discountPrice": 30,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "LoungeWear",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Fleece",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28082020/pexels-photo-28082020/free-photo-of-man-standing-in-green-jacket.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Relaxed Fit Sweatpants Front View",
        "imageId": ""
      }
    ],
    "rating": 4.3,
    "numReviews": 14
  },
  {
    "name": "Formal Dress Pants",
    "description": "Classic formal dress pants with a slim fit. Made from lightweight, wrinkle-resistant fabric for a polished look at the office or formal events.",
    "price": 70,
    "discountPrice": 60,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "ElegantStyle",
    "sizes": [
      "M",
      "L",
      "XL"
    ],
    "material": "Polyester",
    "gender": "Men",
    "images": [
      {
        "url": "https://images.pexels.com/photos/4066290/pexels-photo-4066290.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Formal Dress Pants Front View",
        "imageId": ""
      }
    ],
    "rating": 4.9,
    "numReviews": 8
  },
  {
    "name": "High-Waist Skinny Jeans",
    "description": "High-waist skinny jeans in stretch denim with a button and zip fly. Features a flattering fit that hugs your curves and enhances your silhouette.",
    "price": 50,
    "discountPrice": 45,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "DenimStyle",
    "sizes": [
      "XS",
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Denim",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/26548174/pexels-photo-26548174/free-photo-of-woman-sitting-with-eyes-closed-on-seashore.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "High-Waist Skinny Jeans",
        "imageId": ""
      }
    ],
    "rating": 4.8,
    "numReviews": 20
  },
  {
    "name": "Wide-Leg Trousers",
    "description": "Flowy, wide-leg trousers with a high waist and side pockets. Perfect for an elegant look that combines comfort and style.",
    "price": 60,
    "discountPrice": 55,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "ElegantWear",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Polyester",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/9159074/pexels-photo-9159074.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Wide-Leg Trousers Front View",
        "imageId": ""
      }
    ],
    "rating": 4.7,
    "numReviews": 15
  },
  {
    "name": "Stretch Leggings",
    "description": "Soft, stretch leggings in a high-rise style. Perfect for lounging, working out, or casual wear, with a smooth fit that flatters your body.",
    "price": 25,
    "discountPrice": 20,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "ComfyFit",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton Blend",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/9725199/pexels-photo-9725199.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Stretch Leggings Front View",
        "imageId": ""
      }
    ],
    "rating": 4.5,
    "numReviews": 30
  },
  {
    "name": "Pleated Midi Skirt",
    "description": "Elegant pleated midi skirt with a high waistband and soft fabric that drapes beautifully. Ideal for both formal and casual occasions.",
    "price": 55,
    "discountPrice": 50,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "ChicStyle",
    "sizes": [
      "S",
      "M",
      "L"
    ],
    "material": "Polyester",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28971788/pexels-photo-28971788/free-photo-of-confident-young-adult-woman-outdoors.png?auto=compress&cs=tinysrgb&w=800",
        "altText": "Pleated Midi Skirt Front View",
        "imageId": ""
      }
    ],
    "rating": 4.6,
    "numReviews": 18
  },
  {
    "name": "Flared Palazzo Pants",
    "description": "High-waist palazzo pants with a loose, flowing fit. Comfortable and stylish, making them perfect for casual outings or beach days.",
    "price": 45,
    "discountPrice": 40,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "BreezyVibes",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Linen Blend",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28969172/pexels-photo-28969172/free-photo-of-fashionable-woman-posing-by-a-lakeside-dock.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Flared Palazzo Pants Front View",
        "imageId": ""
      }
    ],
    "rating": 4.4,
    "numReviews": 22
  },
  {
    "name": "High-Rise Joggers",
    "description": "Comfortable high-rise joggers with an elastic waistband and drawstring for a perfect fit. Great for lounging or working out.",
    "price": 40,
    "discountPrice": 35,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "ActiveWear",
    "sizes": [
      "XS",
      "S",
      "M",
      "L"
    ],
    "material": "Cotton Blend",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28888549/pexels-photo-28888549/free-photo-of-confident-woman-sitting-in-a-studio-setting.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "High-Rise Joggers Front View",
        "imageId": ""
      }
    ],
    "rating": 4.3,
    "numReviews": 25
  },
  {
    "name": "Paperbag Waist Shorts",
    "description": "Stylish paperbag waist shorts with a belted waist and wide legs. Perfect for summer outings and keeping cool in style.",
    "price": 35,
    "discountPrice": 30,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "SunnyStyle",
    "sizes": [
      "S",
      "M",
      "L"
    ],
    "material": "Cotton",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28905341/pexels-photo-28905341/free-photo-of-young-woman-enjoying-a-sunny-day-in-the-city.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Paperbag Waist Shorts Front View",
        "imageId": ""
      }
    ],
    "rating": 4.5,
    "numReviews": 19
  },
  {
    "name": "Stretch Denim Shorts",
    "description": "Comfortable stretch denim shorts with a high-waisted fit and raw hem. Perfect for pairing with your favorite tops during warmer months.",
    "price": 40,
    "discountPrice": 35,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "DenimStyle",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Denim",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/28570315/pexels-photo-28570315/free-photo-of-young-woman-with-tablet-and-headphones.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Stretch Denim Shorts Front View",
        "imageId": ""
      }
    ],
    "rating": 4.7,
    "numReviews": 15
  },
  {
    "name": "Culottes",
    "description": "Wide-leg culottes with a flattering high waist and cropped length. The perfect blend of comfort and style for any casual occasion.",
    "price": 50,
    "discountPrice": 45,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "ChicStyle",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Polyester",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/23287387/pexels-photo-23287387/free-photo-of-young-woman-in-a-fashionable-outfit-standing-on-a-shore-in-summer.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Culottes Front View",
        "imageId": ""
      }
    ],
    "rating": 4.6,
    "numReviews": 23
  },
  {
    "name": "Classic Pleated Trousers",
    "description": "Timeless pleated trousers with a tailored fit. A wardrobe essential for workwear or formal occasions.",
    "price": 70,
    "discountPrice": 65,
    "isAvailable": true,
    "category": "Bottom Wear",
    "brand": "ElegantWear",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Wool Blend",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/3538028/pexels-photo-3538028.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Classic Pleated Trousers Front View",
        "imageId": ""
      }
    ],
    "rating": 4.8,
    "numReviews": 20
  },
  {
    "name": "Knitted Cropped Top",
    "description": "A stylish knitted cropped top with a flattering fitted silhouette. Perfect for pairing with high-waisted jeans or skirts for a casual look.",
    "price": 40,
    "discountPrice": 35,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "ChicKnit",
    "sizes": [
      "S",
      "M",
      "L"
    ],
    "material": "Cotton Blend",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/16055235/pexels-photo-16055235/free-photo-of-young-woman-in-a-knitted-top-and-a-pendant-on-her-neck.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Knitted Cropped Top",
        "imageId": ""
      }
    ],
    "rating": 4.6,
    "numReviews": 15
  },
  {
    "name": "Boho Floral Blouse",
    "description": "Flowy boho blouse with floral patterns, featuring a relaxed fit and balloon sleeves. Ideal for casual summer days.",
    "price": 50,
    "discountPrice": 45,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "BohoVibes",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Viscose",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/14363172/pexels-photo-14363172.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Boho Floral Blouse",
        "imageId": ""
      }
    ],
    "rating": 4.7,
    "numReviews": 20
  },
  {
    "name": "Casual T-Shirt",
    "description": "A soft, breathable casual t-shirt with a classic fit. Features a round neckline and short sleeves, perfect for everyday wear.",
    "price": 25,
    "discountPrice": 20,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "ComfyTees",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/1454181/pexels-photo-1454181.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Casual T-Shirt",
        "imageId": ""
      }
    ],
    "rating": 4.5,
    "numReviews": 25
  },
  {
    "name": "Off-Shoulder Top",
    "description": "An elegant off-shoulder top with ruffled sleeves and a flattering fit. Ideal for adding a touch of femininity to your outfit.",
    "price": 45,
    "discountPrice": 40,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "Elegance",
    "sizes": [
      "S",
      "M",
      "L"
    ],
    "material": "Polyester",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/8612758/pexels-photo-8612758.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Off-Shoulder Top",
        "imageId": ""
      }
    ],
    "rating": 4.7,
    "numReviews": 18
  },
  {
    "name": "Lace-Trimmed Cami Top",
    "description": "A delicate cami top with lace trim and adjustable straps. The lightweight fabric makes it perfect for layering or wearing alone during warmer weather.",
    "price": 35,
    "discountPrice": 30,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "DelicateWear",
    "sizes": [
      "S",
      "M",
      "L"
    ],
    "material": "Silk Blend",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/11931375/pexels-photo-11931375.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Lace-Trimmed Cami Top",
        "imageId": ""
      }
    ],
    "rating": 4.8,
    "numReviews": 22
  },
  {
    "name": "Graphic Print Tee",
    "description": "A trendy graphic print tee with a relaxed fit. Pair it with jeans or skirts for a cool and casual look.",
    "price": 30,
    "discountPrice": 25,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "StreetStyle",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/1188748/pexels-photo-1188748.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Graphic Print Tee",
        "imageId": ""
      }
    ],
    "rating": 4.6,
    "numReviews": 30
  },
  {
    "name": "Ribbed Long-Sleeve Top",
    "description": "A cozy ribbed long-sleeve top that offers comfort and style. Perfect for layering during cooler months.",
    "price": 55,
    "discountPrice": 50,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "ComfortFit",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton Blend",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/7656433/pexels-photo-7656433.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Ribbed Long-Sleeve Top",
        "imageId": ""
      }
    ],
    "rating": 4.7,
    "numReviews": 26
  },
  {
    "name": "Ruffle-Sleeve Blouse",
    "description": "A lightweight ruffle-sleeve blouse with a flattering fit. Perfect for a feminine touch to any outfit.",
    "price": 45,
    "discountPrice": 40,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "FeminineWear",
    "sizes": [
      "S",
      "M",
      "L"
    ],
    "material": "Viscose",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/874707/pexels-photo-874707.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Ruffle-Sleeve Blouse",
        "imageId": ""
      }
    ],
    "rating": 4.5,
    "numReviews": 19
  },
  {
    "name": "Classic Button-Up Shirt",
    "description": "A versatile button-up shirt that can be dressed up or down. Made from soft fabric with a tailored fit, it's perfect for both casual and formal occasions.",
    "price": 60,
    "discountPrice": 55,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "ClassicStyle",
    "sizes": [
      "S",
      "M",
      "L",
      "XL"
    ],
    "material": "Cotton",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/1375849/pexels-photo-1375849.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "Classic Button-Up Shirt",
        "imageId": ""
      }
    ],
    "rating": 4.8,
    "numReviews": 25
  },
  {
    "name": "V-Neck Wrap Top",
    "description": "A chic v-neck wrap top with a tie waist. Its elegant style makes it perfect for both casual and semi-formal occasions.",
    "price": 50,
    "discountPrice": 45,
    "isAvailable": true,
    "category": "Top Wear",
    "brand": "ChicWrap",
    "sizes": [
      "S",
      "M",
      "L"
    ],
    "material": "Polyester",
    "gender": "Women",
    "images": [
      {
        "url": "https://images.pexels.com/photos/2050291/pexels-photo-2050291.jpeg?auto=compress&cs=tinysrgb&w=800",
        "altText": "V-Neck Wrap Top",
        "imageId": ""
      }
    ],
    "rating": 4.7,
    "numReviews": 22
  }
]

// Transform all products to have only one size and one color
const transformedProducts = products.map(product => {
  const newProduct = { ...product };
  // Normalize brand
  if (!ALLOWED_BRANDS.includes(newProduct.brand)) {
    newProduct.brand = getRandomAllowed(ALLOWED_BRANDS);
  }
  // Normalize material
  if (!ALLOWED_MATERIALS.includes(newProduct.material)) {
    newProduct.material = getRandomAllowed(ALLOWED_MATERIALS);
  }
  // Normalize size
  if (Array.isArray(newProduct.sizes)) {
    newProduct.size = getRandom(newProduct.sizes);
    delete newProduct.sizes;
  }
  return newProduct;
});

module.exports = transformedProducts;

